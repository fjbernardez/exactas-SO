...
{
	//TODO: Ejercicio 2
	unsigned int inode_group =  blockgroup_for_inode(inode_number);
	unsigned int inode_group_index = blockgroup_inode_index(inode_number);
	Ext2FSBlockGroupDescriptor * block_group_descriptors = block_group(inode_group);	
	unsigned int inode_table = block_group_descriptors->inode_table;
	unsigned char buff [1024];

	read_block(inode_table+(inode_group_index/(1024 / sizeof(Ext2FSInode))),buff);
	Ext2FSInode * as_inode = (Ext2FSInode *) malloc(sizeof(Ext2FSInode));
	Ext2FSInode * buffer_as_inode = (Ext2FSInode *) buff;
	memcpy(as_inode,&buffer_as_inode[inode_group_index%(1024 / sizeof(Ext2FSInode))],sizeof(Ext2FSInode));

	std::cerr << "inode: " << *as_inode<< std::endl;
	return as_inode;
}
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	if (block_number < 12){
		...
		if(block_number>=12 && block_number < (256+12)){
			unsigned char simple [1024];
			read_block(inode->block[12],simple);
			res = (unsigned int)simple[block_number-12];
			unsigned int * simple_int;
			simple_int = (unsigned int *) simple;
			res = simple_int[block_number-12];
		}
		else{
			unsigned char simple [1024];
			read_block(inode->block[13],simple);
			unsigned int * simple_int;
			simple_int = (unsigned int *) simple;
			unsigned int double_number = simple_int[(block_number-268)/256];
			unsigned char double_ind [1024];
			read_block(double_number,double_ind);
			unsigned int * double_int;
			double_int = (unsigned int *) double_ind;
			res = double_ind[(block_number-268)%256];
		}
	}
	return res;

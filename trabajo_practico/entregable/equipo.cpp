...
void Equipo::jugador(int nro_jugador) {
    ...
    while (!this->belcebu->termino_juego()) { // Chequear que no haya una race condition en gameMaster
        ...
        if (!this->belcebu->termino_juego()) {
            ...
            switch (this->strat) { //SECUENCIAL,RR,SHORTEST,USTEDES
        ...
        while (i < tam_Y) {
            ...
            if (color_mirando == bandera_contraria) {
                ...
                string print = "El equipo " + col +
                               ...
                               to_string(finish.tv_nsec - start.tv_nsec) + "nsec en la posicion " +
                               to_string(pos_mirando.first) + " " + to_string(pos_mirando.second);
                cout << print << endl;
...
void Equipo::jugar_turno_estrategia_rr(int nro_jugador) {
    ...
    while (quantum_que_habra_cuando_me_toque > 0) {
        ...
        // Aca se esperaría que haya solo un jugador por vez no debería ser necesario el mutex
...
int Equipo::jugador_moverse(int nro_jugador) {
    ...
    if (!this->belcebu->termino_juego() || probando_coordenada == this->pos_bandera_contraria) {
        ...
        cout << print << endl;
        string print2 = "Me movi a la posicion " + to_string(probando_coordenada.first) + " " +
                        to_string(probando_coordenada.second);
        cout << print2 << endl;
    }
